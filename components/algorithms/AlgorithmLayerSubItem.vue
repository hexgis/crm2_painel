<template>
    <div>
        <v-list-item>
            <v-list-item-icon v-if="type == 'button'"></v-list-item-icon>
        ...
        <v-dialog v-model="showDialogError" width="500">
            <v-card>
                <v-card-title class="headline text-break">
                    {{ $t('error-title') }}
                </v-card-title>

                <v-card-text class="py-6"> {{ $t('error-text') }} </v-card-text>
            </v-card>
        </v-dialog>
    </div>
</template>

    ...
    <div>
        <p class="text-caption text-right text--secondary mr-2 mt-2 mb-1">
            <template v-if="!paused">
                ...
                <v-btn icon small @click="pauseConnection">
                    <v-icon> mdi-pause </v-icon>
                </v-btn>
            </template>
            <v-btn v-else icon small @click="resumeConnection">
                <v-icon> mdi-play </v-icon>
            </v-btn>
            {{ results.length }} {{ $t('results') }}
        </p>

        <v-card v-for="(result, i) in results" :key="i" tile outlined>
            <v-card-text>
                <v-row align="center">
                    <v-col cols="6">
                        <v-img
                    ...
                    <v-col cols="6" class="pl-2">
                        ...
                        <div class="mb-1">Camera: {{ result.camera.name }}</div>
                        <div class="mb-1">
                            Frame:
                            <a
                        ...
                        <div
                            ...
                            class="mb-1"
                        >
                            <span v-if="value" class="text-capitalize">
                                {{ name }}:
                                {{
                                    typeof value === 'object'
                                        ? value.name

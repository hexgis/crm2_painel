      ...
      (aggs, agg) => {
        ...
        if (year === state.params.year) {
          if (month === state.params.month) {
            state.esBuckets.rates[`${year}-${month}`] = (state.esBuckets.rates[`${year}-${month}`] || 0)
                            + agg.area.value;
        ...
        if (parseInt(month) <= parseInt(state.params.month)) {
          state.esBuckets.rates[year] = (state.esBuckets.rates[year] || 0) + agg.area.value;
        }
  ...
  generateAgroColors(state) {
    ...
    states.forEach((stt) => {
      if (!state.colors.state[stt]) {
        state.colors.state[stt] = `#${((Math.random() * 0xFFFFFF) << 0).toString(16)}`;
      }
    });
  },
  ...
  generateDataColors(state) {
    ...
    systems.forEach((system) => {
      if (!state.colors.type[system]) {
        state.colors.type[system] = `#${((Math.random() * 0xFFFFFF) << 0).toString(16)}`;
      }
    ...
    if (!state.colors.year[year]) {
      state.colors.year[year] = `#${((Math.random() * 0xffffff) << 0).toString(16)}`;
    }

    if (!state.colors.year[previousYear]) {
      state.colors.year[previousYear] = `#${((Math.random() * 0xFFFFFF) << 0).toString(16)}`;
    }
  },
};

...
export const actions = {
    ...
    async getDataUsage({ commit }) {
        try {
            const response = await this.$apiSkynet.$get('client/data-usage')

            if (response) commit('setDataUsage', response)
        } catch (error) {
            commit('setDataUsage', {consumed: 0, max: 0})
            commit(
                    'alert/addAlert',
                    { message: this.$i18n.t('error-getting-usage') },
                    { root: true }
                )
        }
    },

    async clearDataUsage({ commit }) {
        commit('setDataUsage', null)
    },
}
